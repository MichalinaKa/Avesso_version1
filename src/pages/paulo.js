...
const PauloPage = ({ data }) => (
  <Layout>
    ...
    <MainWrapper>
      <NavMainWrapper>
        <Link to={data.allDataJson.edges[19].node.url}>
          {" "}
          <span> &larr;</span>
        </Link>

        <Link to="/"> home </Link>
        <Link to={data.allDataJson.edges[21].node.url}>
          {" "}
          <span> &rarr;</span>
        </Link>
      </NavMainWrapper>
    ...
    <Link to={data.allDataJson.edges[20].node.url} className="scrollTopButton">
      {" "}
      <span> &larr;</span>
      to the top
    </Link>
